...
def infer():
    ...
    in_pt1_path = r"D:\Temp\tmp2\DTLN_0108_wMSE_dnsdrb_quater_rts0.4_pre1ms_finetune_NoFactor_ep100_p1.pt"
    in_pt2_path = r"D:\Temp\tmp2\DTLN_0108_wMSE_dnsdrb_quater_rts0.4_pre1ms_finetune_NoFactor_ep100_p2.pt"
    out_wav_path = Path(
        r"D:\Temp\tmp2", f"{Path(in_wav_path).stem};{Path(in_pt1_path).stem};ccc.wav"
    ).as_posix()
    ############# splitter ############
    net1 = torch.jit.load(in_pt1_path)
    net2 = torch.jit.load(in_pt2_path)
    net1.eval()
    net2.eval()
    ...
    with wave.Wave_write(out_wav_path) as fp:
        ...
        for idx, data in enumerate(tqdm(gen), 1):
            ...
            out_mag, in_state1 = net1(mag, in_state1)

            s1_stft = out_mag * torch.exp(1j * phase)
            y1 = torch.fft.irfft(s1_stft)
            y1 = torch.permute(y1, (0, 2, 1))

            out, in_state2 = net2(y1, in_state2)
