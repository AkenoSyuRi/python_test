...
class GRUC_MNN_Infer_Wrapper:
    def __init__(self, model_path):
        ...
        self.estimated_mag_tensor = self.interpreter.getSessionOutput(
            self.session, "27"
        )
    ...
    def forward1(self, mag, states):
        ...
        mag = MNN.Tensor(
            self.mag_tensor.getShape(),
            MNN.Halide_Type_Float,
            mag,
            MNN.Tensor_DimensionType_Caffe,
        )
        states = MNN.Tensor(
            self.states_tensor.getShape(),
            MNN.Halide_Type_Float,
            states,
            MNN.Tensor_DimensionType_Caffe,
        )
        ...
        estimated_mag = MNN.Tensor(
            self.estimated_mag_tensor.getShape(),
            MNN.Halide_Type_Float,
            MNN.Tensor_DimensionType_Caffe,
        )
        h_states = MNN.Tensor(
            self.h_states_tensor.getShape(),
            MNN.Halide_Type_Float,
            MNN.Tensor_DimensionType_Caffe,
        )
...
def get_c_float_arr(*data_list):
    with StringIO() as stream:
        for i, arr in enumerate(data_list):
            line = (
                f"float arr{i}[] {{" + "f,".join(arr.reshape(-1).astype(str)) + "f};\n"
            )
            stream.write(line)
